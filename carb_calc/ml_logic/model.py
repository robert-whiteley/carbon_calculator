...
def prediction(processed_image, model1, model2):
    ...
    embeddings = model1(processed_image)
    X = embeddings.last_hidden_state
    X = X[:,0,:]

    print("✅ extracted embeddings from base_model")

    output = model2.predict(X)

    print("✅ tuned_model prediction run")

    classification = [classes[np.argmax(out)] for out in output]
